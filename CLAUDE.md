...
- `raw/` — immutable source material (articles, notes, transcripts, journal entries), tracked in this repo. Once a file lands here it is never edited by Claude — if something changes, add a new file or a follow-up note rather than modifying the original. Images/attachments go in `raw/assets/`.
